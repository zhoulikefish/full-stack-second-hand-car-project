...
# from .myextend import pro  # 调用API获取代理IP池
import random
import time
import logging
...
#     username = "d3775608033"
#     password = "dvkhwm1d"
#
#     def process_request(self, request, spider):
#         proxy = random.choice(pro.proxy_list)
#         # 用户名密码认证(私密代理/独享代理)
#         request.meta['proxy'] = "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": self.username, "pwd": self.password,
#                                                                         "proxy": proxy}
#         # 白名单认证(私密代理/独享代理)
...
#         # 当请求发生异常时，从代理IP列表中选择一个新的代理IP进行请求
...
#                                                                         "proxy": proxy}
#         # 打印日志或进行其他处理
#         spider.logger.info(f"Request failed, switching to new proxy: {proxy}")
#         return None
...
class RetryMiddleware:
    ...
    def from_crawler(cls, crawler):
        middleware = cls()
        crawler.signals.connect(middleware.retry_failed, signal=signals.request_retried)
        return middleware

    def retry_failed(self, request, reason, spider):
        # 处理重试失败的请求
        spider.logger.info(f"Request failed after retries: {request.url}")
        username = "d3775608033"
        password = "dvkhwm1d"
        proxy = random.choice(pro.proxy_list)
        # 用户名密码认证(私密代理/独享代理)
