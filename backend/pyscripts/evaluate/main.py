...
if len(sys.argv) >= 7:
    ...
    brand = sys.argv[3]
    model = sys.argv[4]
    update_year = sys.argv[5]
    update_monoth = sys.argv[6]
...
def load_models():
    model_xgb_min = joblib.load("model/model_xgb_min.pkl")

    model_lgb_min = joblib.load("model/model_lgb_min.pkl")
    model_xgb_max = joblib.load("model/model_xgb_max.pkl")
    model_lgb_max = joblib.load("model/model_lgb_max.pkl")
    return model_xgb_min, model_lgb_min, model_xgb_max, model_lgb_max

def load_weights():
    with open("model/weights.pkl", "rb") as file:
        weight_xgb_min, weight_lgb_min, weight_xgb_max, weight_lgb_max = pickle.load(file)
    return weight_xgb_min, weight_lgb_min, weight_xgb_max, weight_lgb_max
...
def predict_price(vehicle_info):
    ...
    model_xgb_min, model_lgb_min, model_xgb_max, model_lgb_max = load_models()
    model_xgb_min._n_classes = 1
    model_lgb_min._n_classes = 1
    model_xgb_max._n_classes = 1
    model_lgb_max._n_classes = 1
    weight_xgb_min, weight_lgb_min, weight_xgb_max, weight_lgb_max = load_weights()
    
    # 进行预测的处理...
    y_xgb_min = model_xgb_min.predict(vehicle_info)
    y_lgb_min = model_lgb_min.predict(vehicle_info)
    y_xgb_max = model_xgb_max.predict(vehicle_info)
    y_lgb_max = model_lgb_max.predict(vehicle_info)
    
    
    
    # results ensemble
    predict_y_xlgb_max = y_xgb_min * weight_xgb_min + y_lgb_min * weight_lgb_min  # weighted average
    predict_y_xlgb_min = y_xgb_max * weight_xgb_max + y_lgb_max * weight_lgb_max  # weighted average

    # 判断结果是否在正常范围内
    if(predict_y_xlgb_min<0):
        predict_y_xlgb_min = 0
    
    if(predict_y_xlgb_min>predict_y_xlgb_max):
        predict_y_xlgb_max = predict_y_xlgb_min+5
        
    # feature importance
    ranked_min_feat = rank_feature_importance(model_xgb_min, model_lgb_min, weight_xgb_min, weight_lgb_min, vehicle_info)
    ranked_max_feat = rank_feature_importance(model_xgb_max, model_lgb_max, weight_xgb_max, weight_lgb_max, vehicle_info)
    ...
    ranked_feat = ranked_feat.sort_values('Importance', ascending=False)
    ranked_feat = ranked_feat.drop('Importance', axis = 1 ,inplace=False)
    # print(ranked_feat)
    ...
    return predict_y_xlgb_min, predict_y_xlgb_max, ranked_feat

def read_input():
    # input_list = {
    #     'brand':[input(f"请输入品牌（brand）的值: ")], 
...
# 
    ...
    if len(sys.argv) >= 7:
        ...
        data['update_month'] = update_month
...
def preprocess_input(data:pd.DataFrame):
    ...
    # data = data.drop(['title', 'update_time','license_time'], axis=1, inplace=False) 
    data = data.drop(['update_time'], axis=1, inplace=False) 
    
    # min_max unique preprocessing step
    # group_by(data, ['brand', 'model', 'displacement', 'drive_model', 'license_year', 'color', 'fuel_standard', 'fuel_type', 'fast_charge_time', 'pure_elec_range', 'battery_capacity'])
    # 'mileage'
    
    # features = [i for i in data.columns if i not in ["price", "min_price", "max_price","SaleID"]]
    features = [i for i in data.columns]
    X_test = data[features]
    X_test= X_test[['brand', 'model', 'mileage', 'transmission', 'emission_standards',
                    'displacement', 'inspection_due', 'insurance_due', 'num_of_transfer',
                    'location', 'engine', 'color', 'fuel_standard', 'drive_model',
                    'highlight', 'description', 'image', 'fuel_type', 'pure_elec_range',
                    'fast_charge_time', 'battery_capacity', 'slow_charge_time',
                    'license_year', 'license_month', 'update_year', 'update_month',
                    'update_day']]
...
if __name__ == "__main__":
    ...
    #
    # lgb_min = models[0]
    # lgb_min._n_classes = 1
    # lgb_min.predict(X_test)
    # print(lgb_min.predict(X_test))
    #
    # lgb_min = models[1]
    ...
    predict_y_xlgb_min, predict_y_xlgb_max, ranked_feat = predict_price(X_test)

    min = predict_y_xlgb_min.item()
    max = predict_y_xlgb_max.item()
    ranked_feat = [item[0] for item in ranked_feat]
    

    print(min) # float
    print(max) # float
    # print(ranked_feat) # list